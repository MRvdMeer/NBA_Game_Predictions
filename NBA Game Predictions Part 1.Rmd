...
Here we see there is quite some difference in home-court advantage. Some teams become a lot better at home, while for other teams it barely makes a difference (or even seems to hurt in the case of teams 22 and 26, the Orlando Magic and the Sacramento Kings). Interestingly, there are quite a few teams where home-court advantage is bigger than team skill, or even where team skill is negative and home-court advantage is positive. This model seems to sample better, in the sense that effective sample size is much larger for all parameters than in the previous iterations of this model.
...
Also, we can see that converting probability forecasts into simple win / loss predictions does clearly worse than any of these models. It has a much higher quadratic loss, and its log loss is infinite, because the log loss penalizes a wrong prediction extremely (i.e. a single prediction of the home team winning while it actually loses leads to a loss of intinity).


## Modeling the season as a time series

So far we've been modeling the team skills as a single unobserved variable. This doesn't take into account that a team's skill can change over the course of a season. For instance, a team might acquire a new player or lose one due to injury. We can take this into account by building a model that re-estimates every teams skill after each game they play. In this case we will only expand on the simple home-court advantage model, as it predicts slightly better than the other two models.
