...
One more thing we can try is to allow each team to have a different home court advantage. This makes sense because some teams play at a high altitute or might have other factors that make it particularly difficult for visiting teams to play well.
...
We see quite a different story from earlier - here there seems to be a much clearer home court advantage. Maybe the reason this wasn't obvious with the score difference models is that teams may play at a different pace at home or on the road. As a result they may score more points but the opponents as well. We'll keep this in mind for potential model expansion later.
...
stan_fit_norm_ts <- sampling(stan_model_norm_ts_comp, data = stan_data_ts, iter = n_sims,
...
stan_fit_pois_ts <- sampling(stan_model_pois_ts_comp, data = stan_data_ts, iter = n_sims,
                          control = list(max_treedepth = 15))
```
