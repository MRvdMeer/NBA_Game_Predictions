...
mean(game_data$score_diff)
sd(game_data$score_diff)

mean(game_data$pts_away)
var(game_data$pts_away)
mean(game_data$pts_home)
var(game_data$pts_home)
...
ggplot(data = game_data) +
  geom_density(mapping = aes(x = pts_away), color = "red") +
  geom_density(mapping = aes(x = pts_home), color = "blue")
```


It appears the observed score difference is bimodal, and there appears to be some home-court advantage. We will use this in the model later.
...
stan_model_home_comp <- stan_model(file = "Stan_models/NBA_model_poisson.stan")
